...
class map_navigation():
    ...
    def __init__(self, res = 0.05, size= (20,20), origin = (-10, -10.24), threshold = 0.65):
    ...
    def place_object_at(self, object_found: object_data):
        ...
        centre = self.to_grid(object_found.x_location, object_found.y_location, self.origin_, self.size, self.res)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            cur_pos = self.to_grid(self.pos.x, self.pos.y, self.origin_, self.size, self.res)
            ...
            for i in range(len(points)):
                ...
                endpos = self.to_grid(self.pos.x + (points[i]) * -math.sin(rads), self.pos.y + (points[i]) * math.cos(rads), self.origin_, self.size, self.res)
